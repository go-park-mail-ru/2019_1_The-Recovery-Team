...
const (
	QueryProfiles                   = `SELECT nickname, avatar, record, win, loss FROM profile ORDER BY record`
	...
	QueryProfileByEmailWithPassword = `SELECT id, email, nickname, password FROM profile WHERE LOWER(email) = LOWER($1)`
)
