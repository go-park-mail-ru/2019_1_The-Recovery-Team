...
const (
	QueryProfiles                   = `SELECT nickname, record, win, loss FROM profile`
	QueryProfilesWithLimit          = `SELECT nickname, record, win, loss FROM profile LIMIT $1`
	QueryProfilesWithOffset         = `SELECT nickname, record, win, loss FROM profile OFFSET $1`
	QueryProfilesWithLimitAndOffset = `SELECT nickname, record, win, loss FROM profile LIMIT $1 OFFSET $2 ORDER BY score`
	QueryProfileById                = `SELECT nickname, record, win, loss FROM profile WHERE id = $1`
