...
const (
	...
	QueryProfileByEmail             = `SELECT email FROM profile WHERE email = $1`
	QueryProfileByNickname          = `SELECT nickname FROM profile WHERE nickname = $1`
	...
	QueryProfileByEmailWithPassword = `SELECT id, email, nickname, password, avatar FROM profile WHERE email = $1`
